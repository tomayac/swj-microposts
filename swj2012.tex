...
\textit{While the introduction states the aims of the paper, the actual presentation wavers between describing the application and analyzing the results. Unfortunately this prevents a sufficiently deep analysis/description of either.}

We have focused on describing and interpreting the results, and only present the basics of the application.

\textit{The comparison with existing work appears to be limited.}

We have separated related work in different fields, and treat each field in detail.

\textit{The authors say they tackle their lack of NLP expertise by using multiple NLP services. Did they consider getting the opinion of an NLP expert on the most optimal approach to take?}
...
\textit{Considering an attempt to mask the means for collecting statistics from end users of the application could lead to claims of violating ethics. Additional options are to consider not collecting or masking user identities -- another approach that could increase uptake.}
...
\subsection*{REVIEW 1}
\textit{The problem is a very interesting one, and the methodology is reasonable. However, the evaluation is insufficient and it is not clear what potential benefits this approach may bring.}

We have evaluated the data on an abstract high level in the first part, and provide a concrete low level application of the approach to practical questions in the second part. We have outlined the \emph{revealed insights} explicitly under each practical question and discuss the main consequences of the paradigm shift of focusing on the reader side.

\textit{The data collected for experiment is small. As the authors mention, it is too small to be statistically significant. Therefore, it is difficult to determine whether the findings are useful or to what extent.}

We acknowledge this issue, however, for the current version of the paper have collected six months worth of data, so that the presented results are more representative. We see our approach not as a replacement of traditional social media mining, but as a completion. We hope to establish micropost audience measurement similar to television viewership measurement.

\textit{The authors argue that ``the potential for this data to reveal new insights is promising'' -- it is unclear what this ``new insight'' is. If it refers to the potential interesting topics as reflected by statistics, compared to state-of-the-art, what is the different bits that this method brings?}

We have highlighted new insights explicitly in form of \emph{revealed insights} and dedicated a Section to the discussion of the consequences our approach brings along in contrast to state-of-the-art tradition social media mining techniques. We suggest a concrete comparison of results to concrete questions retrieved through traditional social media mining with results retrieved through our approach in future work.

\textit{The authors argue that the key novelty of this paper is uncovering the Twitter consumers' perspective, however, it is not sufficiently justified.}

In the current version of the paper, we have provided a collection of concrete questions that we answer with our approach and highlight \emph{revealed insights} explicitly. 

\textit{How does this compare to analysis based on producers' perspective? Can you see interesting but different patterns in the data? How does named entity frequencies make a difference from term/hashtag frequencies?} 
...
\textit{Who were the users who participated in the evaluation, and in which way this user sample is representative for the population; what characteristics of the users make them a good choice?}
...
\textit{How is the consumption of tweets influenced by retweets?}
...
\textit{The large parts describing the technical design of the plugin are simply unnecessary.}

We have limited ourselves to the necessary minimum in the current version.

\textit{If the focus was on the plugin itself, then the authors should have justified why the plugin is useful, and what are potential use cases, and motivations for the user to install it.}
...
\textit{Potentially interesting questions to attack in future work: (i) relationship between retweets and views, (ii) relationship between the time of the tweet appearance and the views that the tweet gets (combine that with location and timezone as well)}

Regarding (i), retweet vs. view relations are not focus of the paper. For (ii), we do not measure the number of views a tweet gets (albeit we could technically), however, focus on disambiguated named entities in microposts in general. User location and micropost consumption are indeed an interesting topic, however, also not topic of the paper.

\subsection*{REVIEW 3}
\textit{It would be interesting to analyze with more details the use cases of this tool even if the reader can imagine many applications.}
...
\textit{The main problem with the paper is the weakness of the collected dataset (the consumers have to install this extension and agree to expose their Twitter activities to Google).}
...
\subsection*{REVIEW 4}
\textit{As there is a product called Google Goggles, the title usage of the word Goggles may need to be revised.}

We have removed the referred to term from the title.
