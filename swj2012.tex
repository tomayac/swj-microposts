...
\begin{figure*}
  \centering
    \subfloat[][Twitter]{
      \includegraphics[width=0.40\textwidth, trim=10px 5px 5px 5px, clip]{./resources/twitterschema.png}
      \label{fig:twitterschema}}
    \subfloat[][Facebook]{
      \includegraphics[width=0.40\textwidth, trim=10px 5px 5px 5px, clip]{./resources/facebookschema.png}
...
\textbf{Traditional approach:} using the traditional approach of social media mining, it is impossible to limit the analysis to people who physically were in any of the Arab spring countries during that period of time.
Alternatives, such as only considering geotagged microposts or using user profile data, are insufficient:
consider for instance a foreign journalist who does not geotag microposts and has his profile location set to his hometown.
Additionally, with our approach, only microposts that ever appeared on real users' timelines get analyzed, \emph{i.e.}, only microposts that besides being authored also have found an audience.
...
analogously to the Arab Spring events,
it is impossible to limit the analysis to people who were in Norway at the moment of the attacks.
Since the fraction of people that geotag their micropost in Norway is sparse, and using profile location is error-prone,
only the reader-focused approach allows to retrieve data from people were in Norway at time of the attacks.
We can highlight the posts that affected most Norwegians, \emph{i.e.}, the microposts they have read most. 
...
\textbf{Traditional approach:} a common occasion for mentioning one's telecommunications provider is to check whether only one's own connection is down, or also everyone else's.
Using traditional social media mining, spikes in authoring microposts can be detected.
However, the location of consumers potentially affected by such microposts cannot be determined exactly. 

\begin{table}
    \begin{tabular}{ | l | l | l | l | l |}
    \hline
	\parbox[t]{.6cm}{\textbf{Shah Alam}} & \textbf{Kuching} & \parbox[t]{1cm}{\textbf{Kuala Lumpur}} & \textbf{Keningau} & \textbf{Kulim}\\ \hline 47 & 34 & 18 & 16 & 16\\
    \hline
  \end{tabular}
...
\textbf{Traditional approach:} via traditional social media mining, exclusively the producer side of microposts can be examined.
Using our approach, only microposts that ever appeared on real users' timelines get analyzed, limiting the impact of spammers' and/or trend riders' accounts.
These accounts typically produce seemingly popular content, which in reality consists of spam messages disguised within trend words, impacting traditional social media mining.
