...
\author[C]{\fnms{Giuseppe} \snm{Rizzo}},
\author[B]{\fnms{Rik} \snm{Van de Walle}},
\author[D]{\fnms{Joaquim} \snm{Gabarro}},
\author[A]{\fnms{Arnaud} \snm{Brousseau}\thanks{A. Brousseau was an intern at Google Germany GmbH at time of writing.}}

\runningauthor{T. Steiner, R. Verborgh, R. Troncy, G. Rizzo \textit{et al.}}
...
In contrast, our approach works through accessing its data via browser extensions directly from the social network users' timeline when they visit their social network of choice via a Web browser.
...
Finally, as our approach allows for cross-network \textit{ambiguity-free} social media mining, we can even propose answers for a question like the following:
...
\subsection{Social Network Data Access via APIs}
...
Common literature typically either focuses on just one network (\emph{e.g.},~\cite{russell201121}), or treats the different networks separately (\emph{e.g.},~\cite{russell2011mining}).
...
\autoref{fig:globalmarkets} shows the percentages of the online population of several countries and their usage of Facebook and Twitter.
...
\subsection{Overview of our Data Retrieval and Enrichment Processes}
...
We use a definition of \emph{named entity} that was coined by Grishman \emph{et al.} as \emph{an information unit described by the name of a person or an organization, a location, a brand, a product, a numeric expression including time, date, money and percent found in a sentence}~\cite{Grishman:COLING96}.
...
This is justified by each site's requirement to manually click a link ``$\mathit{n}$ new stories'' (Facebook) or ``$\mathit{n}$ new tweets'' (Twitter) for new content to appear, rather than auto-updating the timeline.
...
We therefore argue that our approach has a higher precision, at the cost of lower recall.

\subsection{Paper Objective and Structure}
We outline our paper objectives and non-objectives explicitly, where  each objective has a corresponding non-objective in the lists below. In this paper, we \textbf{will}:
\begin{itemize}
	\item perform analyses based on disambiguated named entities;
	\item perform analyses based on IP-address-based reader location detection;
...
\begin{itemize}
	...
	\item perform analyses based on natively geotagged microposts;
...
These extensions were released for free on a Web store for browser extensions with the following description (slightly adapted):
\begin{quotation}
...
We use these APIs in parallel, and, by combining their results~\cite{nerd}, aim at the emergence effect in the sense of Aristotle: \emph{``[\ldots] the totality is not, as it were, a mere heap, but the whole is something besides the parts [\ldots]''}\footnote{Aristotle, Metaphysics, Book H 1045a 8-10.}.
...
The complete named entity reconciliation process is described in~\cite{nerd,steiner_nwesp_2011}.
...
\subsection{Pseudocode of the Browser Extensions}
In the following \autoref{lst:pseudocode}, we provide the pseudocode of the browser extensions, which helps the reader get a better understanding of the involved flow of data.

\begin{lstlisting}[caption={Pseudocode of the browser extensions.},label={lst:pseudocode}]
...
We initially announced the availability of the extensions via Twitter, Facebook, and on our personal blogs, with the objective of reaching an as broad and unbiased audience as possible.
...
The extension descriptions contain full disclosure on the collected data and on the usage of a Web analytics tool, however, do not include a concrete mention (apart from a  remark on entity ranking), that we use the collected data for an experiment.
In addition to that, the extension descriptions do not cross-reference each other, \emph{i.e.}, users are \emph{not} actively encouraged to install both extensions in order to guarantee maximum independence of the experiments.

\subsection{User Demographics} 
As the extensions insert a Web analytics tracking snippet, exact user localization is possible based on the users' current physical location, \emph{i.e.}, completely independent from the origin location users might have registered with Facebook or Twitter, and not to be confused with geotagged microposts.
...
\caption{Location of users of the two browser extensions (May~1 -- November~12, 2011).}
\label{fig:location}
\end{figure*}
\end{comment}
...
However, common sense and empiric knowledge suggest that they are correct.
...
In a~first case, one user during one social networking session reads two different microposts that contain one common named entity (\emph{e.g.}, two consecutive Facebook posts that talk about cats).
...
\subsubsection{Differences in Raw Statistics on Facebook and Twitter}
...
We need to note, however, that Facebook microposts are generated at a lot lower frequency than Twitter microposts, and that Twitter microposts are limited to 140 characters, which has an impact on both precision and recall of the named entity disambiguation process.
...
\subsubsection{Statistic Noise Factors}
...
The core outcome of our experiments is a comparison of the named entities that people \emph{read about} on the social networking sites Facebook and Twitter.
From the absolute distinct number of 18,207 (Twitter) and 54,331 (Facebook) of named entities, we have manually cleaned the list of the top-200 named entities on each social network by removing false positives, and normalizing the representing URIs to DBpedia URIs.
...
For Utøya, this is documented, \emph{e.g.}, in the case of survivor Adrian Pracon (@AdrianPracon on Twitter) in a tweet from Sky News producer @fimackiesky\footnote{Sky News contact Utøya survivor: \url{https://twitter.com/\#!/AdrianPracon/status/94573763500326912}}.
...
\textbf{Traditional approach:} using the traditional approach of social media mining, there is no such option as limiting one's analysis to just people who physically were in Norway at the moment of the attacks. In contrast, one would filter by people that have geotagged their microposts with Norwegian locations (sparse), or that have a Norwegian location in the user profile (error-prone). Using our approach, we can also retrieve data from people who just happened to have been in Norway at time of the attacks and that have not geotagged their microposts (more versatile). Additionally, with our approach, only microposts that ever appeared on real users' timelines get analyzed, \emph{i.e.}, only microposts that besides being authored also have found an audience. 
...
Note that the use of proxy servers, which was widespread during the event, has at most minimally skewed the statistics, since Analytics takes various factors in account to determine a~user's location.
...
\textbf{Traditional approach:} using the traditional approach of social media mining, there is no such option as limiting one's analysis to just people who physically were in any of the Arab spring countries during that period of time. Alternatives would have been to limit to geotagged microposts, or to use user profile data, whereas our approach allows for tracking visitor data as well (for example, microposts from a foreign French journalist who has not geotagged their microposts and who has is French hometown in is user profile). Additionally, with our approach, only microposts that ever appeared on real users' timelines get analyzed, \emph{i.e.}, only microposts that besides being authored also have found an audience.  
...
\textbf{Revealed insight:}
City level analyses allow for fine-grained details on, \emph{e.g.}, brand popularity over time.

\textbf{Traditional approach:} a common occasion for mentioning one's telecommunications provider is to check whether only one's own connection is down, or also everyone else's (for example \url{https://twitter.com/#!/bigred712/status/194086376264843265}). Using traditional social media mining, spikes in authoring microposts can be detected, however, not the exact location of potentially affected consumers of such microposts. 
...
\textbf{Traditional approach:} via traditional social media mining, exclusively the producer side of microposts can be examined. Using our approach, only microposts that ever appeared on real users' timelines get analyzed, \emph{i.e.}, only microposts that besides being authored also have found an audience, which limits the impact of spammers and/or trend riders. Such accounts typically produce seemingly popular content, however, only by adding trending terms to their spam messages. Traditional social media mining can be impacted by such microposts.
...
\begin{itemize}
\item \emph{named entities}, which focuses on named entity detection and disambiguation;
\item \emph{semantic annotation of microposts}, which focuses on named entity detection and disambiguation specifically in microposts;
...
The first and most important consequence is the paradigm shift itself.
With our approach, only microposts that ever appeared on real users' timelines get analyzed, \emph{i.e.}, only microposts that besides being authored also have found an audience.
Where the traditional social media mining approach in the lack of geotagged microposts has to fall back on user profile data (for example applied in the BreakingNews.com website's submission page \url{http://www.breakingnews.com/submit#twitter}), with our approach, we can rely on advanced Web analytics data based on very accurate Wi-Fi network and IP-address-based location tracking. 

The second consequence is the availability of only a limited set of micropost data, as illustrated by the Venn diagram in \autoref{fig:vennreadunread}.
...
The fourth consequence is the potential bias introduced by targeting specific Web browsers with the extensions and the willful neglect of desktop and mobile applications. 
