...
\subsection{User Demographics} \label{sec:userdemographics}
...
\autoref{tab:raw} shows the total and unique number of extracted named entities from microposts.
Interesting here is the relation of unique named entities and total named entities.
To clarify the difference between unique and total named entities, we consider the following examples:
(i) one user~A reads two different microposts that contain one common named entity~E (e.g., two consecutive Facebook posts that talk about cats).
In this case, we would track two total named entities, but only one unique named entity.
In a second case (ii) if two users, user~A and user~B, read two different microposts that contain one common named entity, we would still track two total named entities, however, also two unique named entities.
Short, a unique named entity is a named entity that during one visit to a social networking site of one user appears only once.
Looking at the numbers, where we have 76.7\% unique named entities for Twitter and 43.0\% unique named entities for Facebook, we can carefully derive that the reading experience on Twitter is more versatile than on Facebook.
We need to note that Facebook microposts happen a lot less frequently than Twitter microposts and that Twitter microposts are limited to 140 characters.
...
Again, our data is at this point not statistically significant, especially as we have had the interesting phenomenon of having tracked almost more than ten times as many absolute Facebook users than Twitter users, however, seven~day~active user statistics do not fully back this difference (\autoref{sec:userdemographics}).
Clearly further research is necessary that takes the following disruptive factors into account properly:
\begin{itemize}
\item Micropost length and effect on recall and precision of named entity disambiguation.
\item Throughput of microposts per hour and effect on versatility of named entities.
\end{itemize}
...
\begin{table*}
  ...
  \subfloat[][Twitter]{
    ...
    \hline
	\textbf{Total} & \textbf{Unique} & \textbf{Unique / Total (\%)} & \textbf{Avg. Visit Length} \\ \hline
	35,958 & 27,594 & 76.7\% & 00:28:12 \\
    \hline
  \end{tabular}
  ...
  \subfloat[][Facebook]{
    ...
    \hline
	\textbf{Total} & \textbf{Unique} & \textbf{Unique / Total (\%)} & \textbf{Avg. Visit Length} \\ \hline
	316,910 	& 136,196 & 43.0\% & 01:06:07 \\ 
    \hline
  \end{tabular}
  \label{tab:facebookraw}}
  \caption{Raw statistics of total and unique extracted named entities (May~1 -- November~12).}
  \label{tab:raw}
\end{table*}
